...
| Conditional Holder independent of Entry and actual-position state; 1.0.2 per-candidate target binding and post-binding touch | [holder](../src/ai_infra_quant/application/paqs_q_holder.py), [Q01 OHLC and boundary tests](../tests/unit/test_paqs_q_holder_q01.py), [first remediation](reports/TASK_006E_Q_007D_Q01_REMEDIATION.md), [incremental remediation](reports/TASK_006E_Q_007D_Q01_REMEDIATION_02.md) |
...
E01/E02 focused corrections, 1.0.1 identities and current Windows validation are recorded in the
[remediation report](reports/TASK_006C_Q_EVENT_V1_REMEDIATION_01.md). The AVGO observation above
belongs to the historical 1.0.0 run; no new market replay is claimed.
...
| PAQS-018 | TASK-006B1 explicit bounded archive and offline capture replay; current QFQ is not strict historical As-Of | ACTIVE | 2 | REVIEWED_USER_ACCEPTED_INTEGRATED | [Archive tests](../tests/integration/test_market_data_archive_api.py); [report](reports/TASK_006B1_IMPLEMENTATION_REPORT.md) |
...
|---|---|---|
| ARC-001 | Explicit completed D1/M1, validated read-only canonical Security, bounded calendar/quality | [API/service tests](../tests/integration/test_market_data_archive_api.py) |
